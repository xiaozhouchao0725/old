...
fp32 a,b,c,x1,x2;			//一元二次方程系数、结果、目标角度
...
void vision_loop(vision_rxfifo_t *vision_rx)
{
	 fp32 angle_set;
		...
		if(angle_set > 0.08f) angle_set = 0.08f;
		if(angle_set < 1.00f) angle_set = 1.00f;
		...
		if(!vision_rx->rx_update_flag)
		{
				...
				vision_rx->pitch_add =vision_rx->pitch_add * 0.0001f;
		}
		if(vision_rx->rx_update_flag)
		{
		if(fabs(vision_rx->yaw_add_new)>5.0f){
			if(vision_rx->yaw_add_new>0)
				vision_rx->yaw_add_new = 5.0f;
			else if(vision_rx->yaw_add_new<0)	
				vision_rx->yaw_add_new = -5.0f;
		}
		else if(fabs(vision_rx->yaw_add_new)>1.0f){
					if(vision_rx->yaw_add_new>0)
				vision_rx->yaw_add_new = 1.0f;
			else if(vision_rx->yaw_add_new<0)	
				vision_rx->yaw_add_new = -1.0f;
		}
		if(fabs(vision_rx->pitch_add_new)>5.0f){
			if(vision_rx->pitch_add_new>0)
				vision_rx->pitch_add_new = 5.0f;
			else if(vision_rx->pitch_add_new<0)	
				vision_rx->pitch_add_new = -5.0f;
		}
		else if(fabs(vision_rx->pitch_add_new)>1.0f){
					if(vision_rx->pitch_add_new>0)
				vision_rx->pitch_add_new = 1.0f;
			else if(vision_rx->pitch_add_new<0)	
				vision_rx->pitch_add_new = -1.0f;
		}
			vision_rx->yaw_add = vision_rx->yaw_add_new * 0.005f;
			vision_rx->yaw_add = vision_rx->pitch_add_new * 0.005f;
