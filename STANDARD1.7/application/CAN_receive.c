...
//				}
				...
				{
						get_capA.input_voltage = (int16_t)(rx_data[1] << 8 | rx_data[0]);       //ÊäÈëµçÑ¹					
						get_capA.current = (int16_t)(rx_data[3] << 8 | rx_data[2]);             //ÊäÈëµçÁ÷
						get_capA.cap_voltage = (int16_t)(rx_data[5] << 8 | rx_data[4]);         //µçÈÝµçÑ¹
						get_capA.p_set = rx_data[6];                                            //Éè¶¨¹¦ÂÊ
...
/**
  ...
  * @retval         none
  */
void CAN_cmd_shoot(int16_t right, int16_t left, int16_t trigger,int16_t s)
{
    uint32_t send_mail_box;
...
void CAN_cmd_cap(uint16_t temPower)//³¬¼¶µçÈÝ
{
	...
	cap_data.out_auto_en = 1;
	cap_data.wireless_en = 1;
	cap_data.freq_feedback = 5;
